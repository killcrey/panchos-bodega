...
- **Per-product SEO + a real dedicated product page**: `products.slug` (unique, admin-editable, autofilled from title until touched — `wireSlugAutofill` in `main.js`) backs two things. (1) A static file per product at `dist/products/<slug>/index.html`, written by `scripts/prerender-products.js` as an extra `npm run build` step — a clone of the built `index.html` with title/meta description/canonical/OG/Twitter tags and a per-product JSON-LD `Product` block rewritten in, plus `dist/sitemap.xml` (homepage + every product), which is what makes the `Sitemap:` line in `public/robots.txt` resolve to something. This is what non-JS crawlers (Google, social link previews) see. (2) A real in-app product page for humans: `showProductPage` (`main.js`) swaps `#landing-view`/`#store-grid` for `#product-view`, rendering the full untruncated product via `renderProductMarkup`/`wireProductInteractions` — the exact same functions that build a grid card, just called with `linkTitle: false, truncateDescription: false` so the page doesn't link to itself and isn't capped at `MAX_DESCRIPTION_LENGTH` (that cap is a small-tile concern, not a dedicated-page one). `goToProduct` (landing carousel/side-box clicks, and the initial `/products/<slug>/` route match via `matchProductFromPath`) and a card's title-link click both funnel through `showProductPage`. `updateProductUrlAndMeta` — called at the end of `showProductPage` — pushes the URL (no-op if already there) and swaps `document.title`/meta via `setProductMetaTags`; `showStore`/`showLanding` call `resetMetaTagsToHomepage` when leaving a product context. A `popstate` listener re-derives the correct view (product page or landing) on Back/Forward, but doesn't attempt full history fidelity — it won't restore whatever category filter was active before a product was opened.
