...
- **Per-product SEO via prerendered pages**: `products.slug` (unique, admin-editable, autofilled from title until touched — see `wireSlugAutofill` in `main.js`) drives a real static page per product at `dist/products/<slug>/index.html`, written by `scripts/prerender-products.js` as an extra `npm run build` step. It clones the built `index.html` and rewrites the title/meta description/canonical/OG/Twitter tags plus a per-product JSON-LD `Product` block, so Google and non-JS social crawlers see correct per-product content at a real, distinct URL instead of every product resolving to the one homepage. The SPA bundle in that cloned file is unchanged — on load, `matchProductFromPath` in `main.js` detects `/products/<slug>/` and calls `goToProductCard`, so a human visitor still gets the full interactive storefront. Each card's title is a real `<a href="/products/<slug>/">` (`.product-title-link` in `main.js`) — plain left-clicks intercept with `history.pushState` (URL changes, no reload, since the card is already visible); middle-click/ctrl-click/copy-link get the real href untouched, and crawlers get a real followable internal link. `document.title`/meta tags are *not* swapped on that pushState — they only reflect the current product on a hard load of `/products/<slug>/` itself. `scripts/prerender-products.js` also writes `dist/sitemap.xml` (homepage + every product URL), which is what makes the existing `Sitemap:` line in `public/robots.txt` resolve to something real.
