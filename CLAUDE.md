...
- **One shared Resend Audience, fed two ways**: `RESEND_API_KEY` (used everywhere else for transactional email) is a send-only restricted key — confirmed live, it 401s on any Audiences/Contacts call — so contact-list management runs on separate secrets, `RESEND_CONTACTS_API_KEY` + `RESEND_AUDIENCE_ID`. `free-download`'s `addContactToAudience` adds a buyer in real time right after the `email_captures` insert. The Game (`website-assets`, an unrelated Supabase project at `vihwbuiobowdreynxmgy.supabase.co` with no backend of its own — its Resend key would have to live in client-side code, which isn't safe) can't push in real time, so `sync-game-signups` pulls new rows from its `email_signups` table on a `pg_cron` schedule (every 30 min, calling the function via `pg_net` with the public anon key — not a secret, and this function's `verify_jwt = false` like its siblings, since there's nothing sensitive to protect against an early/extra invocation). `game_signup_sync_state` (single-row watermark, `id boolean primary key default true` trick to enforce exactly one row) tracks how far it's read, kept in Bodega's own DB specifically so the sync only ever needs *read* access to the Game's project.
